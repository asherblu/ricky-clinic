 ...
 * 2.4  התמונה של הלוגו עוברת כעת כיווץ (canvas, עד 320px) לפני שמירה ב-localStorage — לא רלוונטי מבחינת
 *      עלות בקובץ הזה (jsx תמיד היה ללא Firebase), אך נשמר לאחידות עם גרסת ה-HTML שעברה מ-Storage ל-Firestore.
 * 2.5  חבילת פיצ'רים גדולה: שדות תאריך לידה/מגדר/קטגוריית גיל במטופל + חישוב גיל אוטומטי; סימון ימי הולדת
 *      בלוח השנה + חלון קופץ בכניסה לאפליקציה ביום הולדת; הדפסת יומן/סטטיסטיקה לפי יום/שבוע/חודש; לשונית
...
function buildGmailComposeLink(to, subject, body, accountIndex) {
  const idx = Number.isFinite(accountIndex) ? accountIndex : 0;
  return `https://mail.google.com/mail/u/${idx}/?view=cm&fs=1&to=${encodeURIComponent(to || "")}&su=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
}
...
function AuthGate({ children }) {
  ...
  const [publicClinicName, setPublicClinicName] = useState(DEFAULT_CLINIC_SETTINGS.clinicName);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await window.storage.get("clinicSettings", false);
        if (res) {
          const parsed = JSON.parse(res.value);
          if (parsed.clinicName) setPublicClinicName(parsed.clinicName);
        }
      } catch { /* keep default */ }
    })();
  }, []);
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Segoe UI', Heebo, Arial, sans-serif", background: COLORS.bg, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
      <div style={{ background: COLORS.surface, borderRadius: 16, padding: "32px 28px", width: "100%", maxWidth: 340, boxShadow: "0 4px 24px rgba(0,0,0,0.08)", textAlign: "center" }}>
        ...
        <div style={{ fontSize: 13, color: COLORS.muted, marginBottom: 22 }}>{publicClinicName}</div>
...
function AppInner({ onRelock }) {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPatients(loaded);
        const todaysBdays = loaded.filter((p) => isBirthdayToday(p.birthDate));
        setBirthdayToday(todaysBdays);
        if (todaysBdays.length > 0 && !birthdayShownRef.current) {
    ...
    (async () => {
      try {
        const res = await window.storage.get("clinicSettings", false);
        if (res) setClinicSettings((prev) => ({ ...prev, ...JSON.parse(res.value) }));
      } catch { /* keep defaults */ }
    })();
  }, []);
  ...
  function saveClinicSettings(next) {
    setClinicSettings(next);
    window.storage.set("clinicSettings", JSON.stringify(next)).catch(() => {});
  }
    ...
    <div dir="rtl" style={{ fontFamily: "'Segoe UI', Heebo, Arial, sans-serif", background: COLORS.bg, minHeight: "100vh", color: COLORS.text }}>
      ...
      <header style={{ background: COLORS.primary, color: "#fff", padding: "14px 20px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <span style={{ fontSize: 18, fontWeight: 700, letterSpacing: 0.3 }}>{clinicSettings.clinicName}</span>
        ...
        <div style={{ flex: 1, display: activeTab === "patients" && !selectedId ? "none" : "block", minWidth: 0 }} className="main-area">
          ...
          {activeTab === "calendar" ? (
            <CalendarView
              ...
              logoUrl={logoUrl} clinicSettings={clinicSettings}
            />
          ) : activeTab === "summary" ? (
      ...
      {showBirthdayModal && birthdayToday.length > 0 && <BirthdayModal patients={birthdayToday} onClose={() => setShowBirthdayModal(false)} />}
      {showReminderSettings && (
        <SettingsModal
          ...
          clinicSettings={clinicSettings} onSaveClinicSettings={saveClinicSettings}
...
function PatientDetail({ patient, onBack, onEdit, onDelete, onAddSession, onAddSessionAt, onEditSession, onDeleteSession, onCancelOccurrence, onEditRecurring, onAddDocument, logoUrl, clinicSettings }) {
  ...
  return (
    <div className="scrollbar" style={{ height: "100%", overflowY: "auto", padding: "20px 28px 80px" }}>
      ...
      {showFile && <PatientFileView patient={patient} logoUrl={logoUrl} clinicSettings={clinicSettings} onClose={() => setShowFile(false)} />}
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", margin: "10px 0 14px", fontSize: 14, color: COLORS.muted }}>
        ...
        {patient.birthDate && (
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}>
            🎂 {formatShortDate(patient.birthDate)} · גיל {calcAge(patient.birthDate)} · {calcAge(patient.birthDate) < 18 ? "ילד/ה" : "מבוגר/ת"}
          </span>
        )}
        {!patient.birthDate && patient.ageCategory && (
          <span style={{ display: "flex", alignItems: "center", gap: 5 }}>{patient.ageCategory === "child" ? "ילד/ה" : "מבוגר/ת"}</span>
        )}
        {patient.gender && <span>{patient.gender === "female" ? "נקבה" : "זכר"}</span>}
      ...
      {(!patient.documents || patient.documents.length === 0) ? (
        ...
        <div style={{ marginBottom: 20 }}>
          {patient.documents.map((doc) => <DocumentRow key={doc.id} doc={doc} patient={patient} clinicSettings={clinicSettings} />)}
        </div>
      )}
    </div>
  );
...
function DocumentRow({ doc, patient, clinicSettings }) {
...
function PatientFileView({ patient, logoUrl, clinicSettings, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#fff", zIndex: 300, overflowY: "auto" }}>
      ...
      <div className="print-area" style={{ padding: "28px 32px", maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ fontSize: 12, color: COLORS.muted, marginBottom: 2 }}>{clinicSettings.clinicName}</div>
        ...
        {tab === "sessions" ? (
          ...
          <FinanceSection billable={billable} scope={financeScope} year={financeYear} month={financeMonth} totalAllTime={totalCharged} patient={patient} clinicSettings={clinicSettings} />
        )}
        {clinicSettings.printFooterText && <p style={{ fontSize: 11, color: COLORS.muted, marginTop: 24, borderTop: `1px solid ${COLORS.border}`, paddingTop: 8 }}>{clinicSettings.printFooterText}</p>}
      </div>
    </div>
  );
}
...
function FinanceSection({ billable, scope, year, month, totalAllTime, patient, clinicSettings }) {
  const [copied, setCopied] = useState(false);
  ...
  const relevant = scope === "month"
    ...
    : billable;
  const emailSubject = renderEmailTemplate(clinicSettings.emailSubjectTemplate, patient, "", clinicSettings.emailSenderName);
  const emailBody = renderEmailTemplate(clinicSettings.emailBodyTemplate, patient, buildPatientBillingText(patient, relevant), clinicSettings.emailSenderName);
  ...
  return (
    <div>
      ...
      <div className="no-print" style={{ display: "flex", gap: 8, flexWrap: "wrap", margin: "10px 0" }}>
        ...
        <a href={buildMailtoLink(patient.email, emailSubject, emailBody)}
        ...
        <a href={buildGmailComposeLink(patient.email, emailSubject, emailBody, clinicSettings.gmailAccountIndex)}
      ...
      <p style={{ fontSize: 12.5, color: COLORS.muted, marginTop: 0, marginBottom: 16 }}>
        כולל פגישות שהתקיימו וביטולים שחויבו בלבד. סה"כ מצטבר לכל הזמן: ₪{totalAllTime}. נוסח המייל ניתן לעריכה בהגדרות ← מייל. "העתקה ופתיחה בחשבונית ירוקה" מכינה טקסט מפורמט לפי פריטים ופותחת את האתר להדבקה ידנית.
      </p>
      {relevant.length === 0 ? (
...
function PatientFormModal({ initial, allPatients, onSave, onClose }) {
  ...
  function doSave(recurring) {
    onSave({ name: name.trim(), phone, email, tags, generalNotes, treatmentGoals, sessionRate: sessionRate === "" ? 0 : Number(sessionRate), emergencyContactName: emergencyContactName.trim(), emergencyContactPhone: emergencyContactPhone.trim(), birthDate, gender, ageCategory, recurring });
  }
    ...
    <Modal title={initial ? "עריכת פרטי מטופל/ת" : "מטופל/ת חדש/ה"} onClose={onClose}>
      ...
      <Field label="תאריך לידה">
        ...
        {birthDate && calcAge(birthDate) !== null && (
          <div style={{ fontSize: 12.5, color: COLORS.muted, marginTop: 4 }}>גיל: {calcAge(birthDate)} · קטגוריה: {calcAge(birthDate) < 18 ? "ילד/ה" : "מבוגר/ת"} (מחושב אוטומטית)</div>
        )}
      </Field>
      <Field label="מגדר">
      ...
      {!birthDate && (
        <Field label="קטגוריית גיל (ידני — כי אין תאריך לידה)">
...
function BirthdayModal({ patients, onClose }) {
  return (
    <Modal title="🎉 יום הולדת היום!" onClose={onClose} small>
      <p style={{ fontSize: 14, color: COLORS.text, lineHeight: 1.7, marginBottom: 16 }}>היום יום ההולדת של:</p>
      <ul style={{ margin: "0 0 18px", paddingRight: 20, fontSize: 15, lineHeight: 2 }}>
        {patients.map((p) => (
          <li key={p.id}><strong>{p.name}</strong>{p.birthDate ? ` (${calcAge(p.birthDate)})` : ""}</li>
        ))}
      </ul>
      <button onClick={onClose} style={btnPrimary()}>תודה 🌿</button>
    </Modal>
  );
}

const DEFAULT_CLINIC_SETTINGS = {
  clinicName: "מרפאה — כרטיסי מטופלים",
  printFooterText: "",
  emailSubjectTemplate: "פרטי חיוב — {שם המטופל}",
  emailBodyTemplate: "שלום {שם המטופל},\n\n{פרטים}\n\nבברכה,\n{שם השולח}",
  emailSenderName: "",
  gmailAccountIndex: 0,
};
function renderEmailTemplate(tpl, patient, detailsText, senderName) {
  return tpl
    .replace(/\{שם המטופל\}/g, patient.name)
    .replace(/\{פרטים\}/g, detailsText)
    .replace(/\{שם השולח\}/g, senderName || "");
}
function calcAge(birthDateStr) {
  ...
  const mth = t.getMonth() - b.getMonth();
  if (mth < 0 || (mth === 0 && t.getDate() < b.getDate())) age--;
  return age;
}
...
function SettingsModal({ settings, onSaveSettings, templates, onSaveTemplates, logoUrl, onUploadLogo, clinicSettings, onSaveClinicSettings, onClose }) {
  ...
  function showPreview() {
    const sample = template
      ...
      .replace("{שעה}", "10:00")
      .replace("{שם המרפאה}", clinicSettings.clinicName);
    setPreview(sample);
  }
  ...
  function handleLogoFile(e) {
    ...
    resizeImageToDataUrl(file)
      .then((dataUrl) => onUploadLogo(dataUrl))
      .catch(() => setLogoError("קריאת הקובץ נכשלה. נסי קובץ תמונה אחר."));
    e.target.value = "";
  }

  // --- פרטי מרפאה / הדפסה / מייל ---
  const [localClinicName, setLocalClinicName] = useState(clinicSettings.clinicName);
  const [printFooterText, setPrintFooterText] = useState(clinicSettings.printFooterText || "");
  const [emailSubjectTemplate, setEmailSubjectTemplate] = useState(clinicSettings.emailSubjectTemplate || DEFAULT_CLINIC_SETTINGS.emailSubjectTemplate);
  const [emailBodyTemplate, setEmailBodyTemplate] = useState(clinicSettings.emailBodyTemplate || DEFAULT_CLINIC_SETTINGS.emailBodyTemplate);
  const [emailSenderName, setEmailSenderName] = useState(clinicSettings.emailSenderName || "");
  const [gmailAccountIndex, setGmailAccountIndex] = useState(clinicSettings.gmailAccountIndex ?? 0);
  const [emailPreview, setEmailPreview] = useState(null);
  function handleSaveClinicName() { onSaveClinicSettings({ ...clinicSettings, clinicName: localClinicName.trim() || DEFAULT_CLINIC_SETTINGS.clinicName }); }
  function handleSavePrint() { onSaveClinicSettings({ ...clinicSettings, printFooterText }); }
  function handleSaveEmail() {
    onSaveClinicSettings({
      ...clinicSettings,
      emailSubjectTemplate, emailBodyTemplate, emailSenderName,
      gmailAccountIndex: Number(gmailAccountIndex) || 0,
    });
  }
  function showEmailPreview() {
    const sampleDetails = "25/04/2024 — ₪350\n02/05/2024 — ₪350\n\nסה\"כ לתשלום: ₪700";
    setEmailPreview({
      subject: renderEmailTemplate(emailSubjectTemplate, { name: "דנה כהן" }, sampleDetails, emailSenderName),
      body: renderEmailTemplate(emailBodyTemplate, { name: "דנה כהן" }, sampleDetails, emailSenderName),
    });
  }
  ...
  return (
    <Modal title="הגדרות המרפאה" onClose={onClose}>
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        ...
        <TabButton label="פרטי מרפאה" active={tab === "clinic"} onClick={() => setTab("clinic")} />
        <TabButton label="הדפסה" active={tab === "print"} onClick={() => setTab("print")} />
        <TabButton label="מייל" active={tab === "email"} onClick={() => setTab("email")} />
        <TabButton label="אודות" active={tab === "about"} onClick={() => setTab("about")} />
      </div>
      ...
      {tab === "logo" && (
        <>
          <p style={{ fontSize: 12.5, color: COLORS.muted, margin: "0 0 14px", lineHeight: 1.5 }}>
            הלוגו יופיע גדול במסך הכניסה, ובגודל קטן בכותרת האפליקציה, בתיק האישי, ובמסמכים. נשמר מקומית בדפדפן הזה בלבד (גרסת artifact).
          </p>
      ...
      {tab === "clinic" && (
        <>
          <p style={{ fontSize: 12.5, color: COLORS.muted, margin: "0 0 14px", lineHeight: 1.5 }}>
            השם הזה מופיע במסך הכניסה, בכותרת האפליקציה, ובתיק האישי — במקום "מרפאה — כרטיסי מטופלים" הגנרי.
          </p>
          <Field label="שם המרפאה / המטפלת">
            <input value={localClinicName} onChange={(e) => setLocalClinicName(e.target.value)} style={inputStyle()} placeholder="לדוגמה: ריקי בלומנפלד — טיפול ומיינדפולנס" />
          </Field>
          <button onClick={handleSaveClinicName} style={btnPrimary()}>שמירת שם המרפאה</button>
        </>
      )}
      ...
      {tab === "print" && (
        <>
          <p style={{ fontSize: 12.5, color: COLORS.muted, margin: "0 0 14px", lineHeight: 1.5 }}>
            הטקסט הזה יופיע כשורת סיום בתחתית תיקים מודפסים. כותרת/כתובת שהדפדפן עצמו מוסיף בהדפסה אינם ניתנים לשליטה מכאן — ניתן לכבות אותם בחלון ההדפסה עצמו, תחת "עוד הגדרות" ← לבטל את הסימון "כותרות ותחתיות".
          </p>
          <Field label="שורת סיום מותאמת אישית (footer) במסמכים מודפסים">
            <textarea value={printFooterText} onChange={(e) => setPrintFooterText(e.target.value)} rows={3} style={{ ...inputStyle(), resize: "vertical" }} />
          </Field>
          <button onClick={handleSavePrint} style={btnPrimary()}>שמירת הגדרות הדפסה</button>
        </>
      )}
      ...
      {tab === "email" && (
        <>
          <p style={{ fontSize: 12.5, color: COLORS.muted, margin: "0 0 14px", lineHeight: 1.5 }}>
            התבניות האלה חלות על מיילים שנשלחים דרך כפתורי "שליחה במייל"/"שליחה ב-Gmail". ניתן להשתמש ב-{"{שם המטופל}"}, {"{פרטים}"} ו-{"{שם השולח}"}.
          </p>
          <Field label="נושא ההודעה"><input value={emailSubjectTemplate} onChange={(e) => setEmailSubjectTemplate(e.target.value)} style={inputStyle()} /></Field>
          <Field label="גוף ההודעה">
            <textarea value={emailBodyTemplate} onChange={(e) => setEmailBodyTemplate(e.target.value)} rows={6} style={{ ...inputStyle(), resize: "vertical" }} />
          </Field>
          <Field label="שם השולח/ת (יופיע בחתימת המייל)">
            <input value={emailSenderName} onChange={(e) => setEmailSenderName(e.target.value)} style={inputStyle()} placeholder="לדוגמה: ריקי בלומנפלד" />
          </Field>
          <Field label="חשבון Gmail לשימוש (אם יש כמה חשבונות מחוברים בדפדפן)">
            <input type="number" min="0" value={gmailAccountIndex} onChange={(e) => setGmailAccountIndex(e.target.value)} style={inputStyle()} />
            <p style={{ fontSize: 11.5, color: COLORS.muted, margin: "6px 0 0", lineHeight: 1.5 }}>
              0 = החשבון הראשון שמחובר בדפדפן, 1 = השני, וכך הלאה.
            </p>
          </Field>
          <button onClick={showEmailPreview} style={{ ...btnPrimary(), width: "auto", background: "transparent", color: COLORS.primary, border: `1px solid ${COLORS.border}`, marginBottom: 14 }}>תצוגה מקדימה</button>
          {emailPreview && (
            <div style={{ background: COLORS.bg, borderRadius: 8, padding: 12, fontSize: 13.5, lineHeight: 1.6, marginBottom: 14 }}>
              <div style={{ fontWeight: 700, marginBottom: 6 }}>נושא: {emailPreview.subject}</div>
              <div style={{ whiteSpace: "pre-wrap" }}>{emailPreview.body}</div>
            </div>
          )}
          <button onClick={handleSaveEmail} style={btnPrimary()}>שמירת הגדרות מייל</button>
        </>
      )}

      {tab === "about" && (
        <div style={{ fontSize: 14, lineHeight: 1.9 }}>
          <div><strong>גרסה:</strong> {APP_VERSION}</div>
          <div><strong>תאריך גרסה:</strong> {formatHeDate(APP_RELEASE_DATE)}</div>
          <div><strong>נוצר על ידי:</strong> {APP_CREATORS}</div>
        </div>
      )}
    </Modal>
  );
...
function CalendarView({ patients, calMonth, setCalMonth, onPickPatient, logoUrl, clinicSettings }) {
  const { y, m } = calMonth;
  ...
  const birthdaysByDay = useMemo(() => {
    ...
    (patients || []).forEach((p) => {
      ...
      if (isNaN(b) || b.getMonth() !== m) return;
      const d = b.getDate();
      map[d] = map[d] || [];
      map[d].push(p.name);
    });
        ...
        {cells.map((d, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              <div style={{ fontSize: 12, fontWeight: isToday ? 800 : 600, color: isToday ? COLORS.clay : COLORS.text, display: "flex", alignItems: "center", gap: 3 }}>
                {d}
                {birthdaysByDay[d] && <span title={`יום הולדת: ${birthdaysByDay[d].join(", ")}`}>🎂</span>}
              </div>
...
function CalendarPrintModal({ patients, logoUrl, clinicSettings, onClose }) {
  ...
  const grouped = useMemo(() => {
    const map = {};
    (patients || []).forEach((p) => {
      getOccurrencesInRange(p, range.start, range.end).forEach((o) => {
        map[o.date] = map[o.date] || [];
        map[o.date].push({ ...o, patientName: p.name });
      });
    });
    Object.values(map).forEach((arr) => arr.sort((a, b) => a.time.localeCompare(b.time)));
    return map;
  ...
  return (
    <Modal title="הדפסת יומן" onClose={onClose}>
      <div className="no-print" style={{ display: "flex", gap: 8, marginBottom: 14, flexWrap: "wrap" }}>
        {[["day", "יום"], ["week", "שבוע"], ["month", "חודש"]].map(([mv, l]) => (
          <TabButton key={mv} label={l} active={mode === mv} onClick={() => setMode(mv)} />
        ))}
      </div>
      <div className="no-print" style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 16 }}>
        <button onClick={() => setAnchor(shiftAnchor(mode, anchor, -1))} style={navBtn()}><ChevronRight size={15} /></button>
        <span style={{ fontSize: 13.5, fontWeight: 600 }}>{formatHeDate(range.start)} – {formatHeDate(range.end)}</span>
        <button onClick={() => setAnchor(shiftAnchor(mode, anchor, 1))} style={navBtn()}><ChevronLeft size={15} /></button>
      </div>
      <div className="print-area" style={{ maxWidth: 700 }}>
        {logoUrl && <img src={logoUrl} alt="לוגו המרפאה" style={{ maxHeight: 48, marginBottom: 12, display: "block" }} />}
        <h2 style={{ fontSize: 18, color: COLORS.primary, marginBottom: 4 }}>{clinicSettings.clinicName}</h2>
        <p style={{ fontSize: 13, color: COLORS.muted, marginBottom: 16 }}>יומן: {formatHeDate(range.start)} – {formatHeDate(range.end)}</p>
        {dates.length === 0 ? <p style={{ color: COLORS.muted }}>אין פגישות בטווח שנבחר.</p> : dates.map((date) => (
          <div key={date} style={{ marginBottom: 14, breakInside: "avoid" }}>
            <h3 style={{ fontSize: 14, color: COLORS.primary, borderBottom: `1px solid ${COLORS.border}`, paddingBottom: 4 }}>{formatHeDate(date)}</h3>
            {grouped[date].map((o) => (
              <div key={o.virtual ? "v-" + date + o.patientName : o.session.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 13.5, padding: "4px 0" }}>
                <span>{o.time} — {o.patientName}{(!o.virtual && o.session.type) ? ` (${o.session.type})` : ""}</span>
                <span style={{ color: o.status === "cancelled" ? COLORS.danger : o.status === "completed" ? COLORS.success : COLORS.clay }}>
                  {o.status === "cancelled" ? "בוטלה" : o.status === "completed" ? "התקיימה" : "מתוכננת"}
                </span>
              </div>
            ))}
          </div>
      ...
      <button onClick={() => window.print()} style={{ ...btnPrimary(), marginTop: 14 }}><Printer size={15} /> הדפסה</button>
    </Modal>
  );
}
function navBtn() {
...
function StatisticsView({ patients, logoUrl, clinicSettings }) {
  ...
  return (
    <div className="scrollbar" style={{ height: "100%", overflowY: "auto", padding: "20px 28px 80px" }}>
      ...
      <div className="no-print" style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <button onClick={() => window.print()} style={{ ...btnPrimary(), width: "auto", background: "transparent", color: COLORS.primary, border: `1px solid ${COLORS.border}` }}><Printer size={15} /> הדפסה</button>
      </div>

      <div className="print-area">
      {logoUrl && <img src={logoUrl} alt="לוגו המרפאה" style={{ maxHeight: 48, marginBottom: 12, display: "block" }} />}
      <h2 style={{ fontSize: 18, color: COLORS.primary, marginBottom: 4 }}>{clinicSettings.clinicName} — דוח סטטיסטיקה</h2>
      <p style={{ fontSize: 13, color: COLORS.muted, marginBottom: 16 }}>{range.label}</p>
