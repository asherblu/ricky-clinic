...
import {
  ...
  User, ArrowRight, Tag, FileText, CalendarPlus, AlertCircle, Copy, Bell, Mic, Square, Printer, BarChart3, Target, ClipboardList, Mail, Lock, Key, Upload, HardDrive
} from "lucide-react";
...
function PatientDetail({ patient, onBack, onEdit, onDelete, onAddSession, onAddSessionAt, onEditSession, onDeleteSession, onCancelOccurrence, onEditRecurring }) {
  const t = todayStr();
  ...
  return (
    <div className="scrollbar" style={{ height: "100%", overflowY: "auto", padding: "20px 28px 80px" }}>
      ...
      {showFile && <PatientFileView patient={patient} onClose={() => setShowFile(false)} />}
...
function PatientFileView({ patient, onClose }) {
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12.5, color: COLORS.muted, marginTop: 0, marginBottom: 16 }}>
        כולל פגישות שהתקיימו וביטולים שחויבו בלבד. סה"כ מצטבר לכל הזמן: ₪{totalAllTime}. "העתקת פרטי חיוב" מכינה טקסט מוכן להדבקה בחשבונית ירוקה או כל מערכת אחרת.
      </p>
      {relevant.length === 0 ? (
...
function PatientFormModal({ initial, allPatients, onSave, onClose }) {
  ...
  function doSave(recurring) {
    onSave({ name: name.trim(), phone, email, tags, generalNotes, treatmentGoals, sessionRate: sessionRate === "" ? 0 : Number(sessionRate), recurring });
  }
